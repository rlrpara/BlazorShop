...
public class UsuarioController : ControllerBase
{
    ...
    [HttpPost()]
    [AllowAnonymous]
    public IActionResult Adicionar([FromBody] UsuarioViewModel model)
    {
        ...
        {
            var usuario = _service.ObterTodos().Where(x => x.Email.Equals(model.Email));
    ...
    public IActionResult Delete(int? id)
    {
        if (id == null)
            return BadRequest(new
            {
                ...
                Dica = "Informe o ID no final da URL. Exemplo: '/api/Usuario/42'"
            });
